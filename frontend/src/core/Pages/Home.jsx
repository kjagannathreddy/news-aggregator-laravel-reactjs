...
const Home = () => {
  ...
  useEffect(() => {
    ...
    const dateIds = dates.map((item) => item.value);

    sendGetRequest(
      `articles?page=${page}&search=${searchTerm}&authors=${authorIds}&sources=${sourceIds}&dates=${dateIds}`,
        ...
        <div className="col-span-1">
          <div className="mt-1">
            <AsyncPaginate
              ...
              dateLoadOptions 
              loadOptions={(search, loadedOptions, { page }) =>
                dateLoadOptions(search, loadedOptions, { page, loggedUser })
              }
              onChange={(selectedValue) => {
                onChangeFilter(selectedValue, "dates");
              }}
              additional={{
                page: 1,
              }}
              isMulti={true}
              styles={customStyles}
              placeholder={<div>Select dates</div>}
            />
          </div>
        </div>
