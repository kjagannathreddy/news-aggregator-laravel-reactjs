...
{
    public function getArticles($search, $authors, $sources, $dates)
    {
        ...
        $this->applyDateFilters($articlesQuery, $dates);
    ...
    private function applyDateFilters($query, $dates)
    {
        if (count($dates)) {
            $query->where(function ($subQuery) use ($dates) {
                $subQuery->whereIn(DB::raw('DATE(news.published_at)'), $dates);
            });
        }
    }
    ...
    {
        $dates = News::selectRaw('DATE(published_at) as date')
            ->groupBy('date');

        return $dates->orderBy('date', 'desc')->simplePaginate(10, ['*'], 'page');
    }
}
